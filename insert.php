...
error_reporting(0);
	...
	if(isset($_POST["documento"]) ){
   	...
   	$doc = str_replace(".", "", $doc1);   ///monto an pgar
		
					//	$url = "localhost:81/asientos/ws/ws001.php?documento=$doc&tipodocumento=1";
						$url = "http://localhost:81/ws/ws001.php?documento=$doc&tipodocumento=1";
						$ch = curl_init($url);
...
$doc1=$_POST['documento'];
$doc = str_replace(".", "", $doc1); 

$sql="SELECT DATE_FORMAT(fecha, '%d-%m-%Y') as fecha,horario ,
     ...
     WHEN 7 THEN 'Sabado'
     END nombre_dia
	FROM filas.servicios where idservicio='$idservicio'";
$r= $mysqli->query($sql);
$re = $r->fetch_object();
$fe= $re->fecha;
$ho= $re->horario;
$dia=$re->nombre_dia;
  ...
  ///verificar si esta registado en este dia
...
where 
    ...
    and estado=1 
    and ci_persona='$doc'  and asistencia !=2 ";
...
?>
      ...
      <td><input type="text" name="ci_persona<?php echo $cantidad;?>" value="<?php echo $doc ?>" readonly required="required"></td>
...
";
    ...
    echo "<script>
    alert('Lo sentimos pero el documento $doc ya se encuentra registrado para este dia')
      history.go(-1);
