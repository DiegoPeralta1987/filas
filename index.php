...
where s.estado=1 and e.condicion=1 and   s.ideventos in(3,72,75) ";
...
INNER JOIN eventos e on s.ideventos=e.ideventos
INNER JOIN stock st on s.idservicio=st.idservicio
where s.estado=1 and e.condicion=1 and s.ideventos!= 3 and s.ideventos !=72 and s.ideventos!=22 and s.ideventos!=28 and s.ideventos!=75 and s.ideventos!=76 and s.ideventos!=2 ORDER BY fecha asc";
...
INNER JOIN eventos e on s.ideventos=e.ideventos
INNER JOIN stock st on s.idservicio=st.idservicio
where s.estado=1 and e.condicion=1 and s.idservicio !=179";
$resultado4=$mysqli->query($query4);


$query5 = "SELECT s.idservicio,  DATE_FORMAT(s.fecha, '%d-%m-%Y') as fecha,  s.horario, e.evento,st.cantidad
FROM filas.servicios s
INNER JOIN eventos e on s.ideventos=e.ideventos
INNER JOIN stock st on s.idservicio=st.idservicio
where s.estado=1 and e.condicion=1   and s.ideventos=2";
		 ...
		 <script language="javascript">
            ...
            $(document).ready(function(){
                $("#cbx_municipio").change(function () {
        		 ...
        		 <script language="javascript">
            $(document).ready(function(){ 
                $("#cbx_estado5").change(function () {

                    $('#cbx_localidad5').find('option').remove().end().append('<option value="whatever"></option>').val('whatever');
                    
                    $("#cbx_estado5 option:selected").each(function () {
                        idservicio = $(this).val();
                        $.post("includes/getStock.php", { idservicio: idservicio }, function(data){
                            $("#cbx_municipio5").html(data);
                        });            
                    });
                })
            });
     ...
     <div class="row">
       <div class="col-md-4">
          ...
          <h4><img src="public/img/icon 4.jpg" />El guardia te controlar&aacute; la temperatura, la cual no debe ser mayor a 37&deg; C.</h4><br>
           <h4> Ni&ntilde;os desde los 4 a&ntilde;os deben inscribirse.</h4><br>
           <h4> Menores de 4 a&ntilde;os deben permanecer con el padre o la madre.</h4><br>
        ...
        <h2>REGISTR&Aacute; TU ASISTENCIA</h2>
            <div class="grid_3 grid_5">
                <div class="bs-example bs-example-tabs" role="tabpanel" data-example-id="togglable-tabs">
                    ...
                    <div class="tab">
 <!-- <button class="tablinks " onclick="openCity(event, 'Cultos')">Cultos</button>
...
<!--  <button class="tablinks" onclick="openCity(event, 'Cultos de 3ra Edad')">Cultos de 3ra Edad</button> -->  
  <button class="tablinks active" onclick="openCity(event, 'Bautismo')">Bautismo</button>
   <button class="tablinks" onclick="openCity(event, 'reservas')">Cancelar Reserva</button>
</div>

<div id="Cultos" class="tabcontent"  >
<br>

<h4>Agendamientos para los cultos ya no es necesario</h4><br>
<h4>Registrate al ingresar en la Iglesia</h4><br>
...
<div id="Bautismo" class="tabcontent" style="display: block;">

<br>
<form id="combo5" name="combo5" action="insert.php" method="POST">
                                        
    <div>
                
            <label><img src="public/img/calendar.png" width="25" height="25"/></i>&nbsp;Selecciona D&iacute;a y Hora: </label>
                
            <select name="cbx_estado" id="cbx_estado5" required class="cajaTexto">
            ...
            <select name="cbx_municipio" id="cbx_municipio5" required></select>
            </div>
        <br>
        <div>   <label><img src="public/img/anotador.png" width="25" height="25"/>&nbsp;C.I. del Participante: </label>
        <input type="text" name="documento" maxlength="15" pattern="[A-Za-z0-9]+" required placeholder="1234567">
